...
class VGG(nn.Module):
    def __init__(self, features, num_classes=1000, init_weights=True):
        ...
        self.nl_2 = NONLocalBlock2D(in_channels=2048)
        self.conv_3 = nn.Sequential(
            nn.Conv2d(in_channels=2048, out_channels=4096, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(4096),
            nn.ReLU(),
            nn.MaxPool2d(2),
        )
    ...
    def forward(self, x):
        ...
        print('use avgpool!')
        x = self.avgpool(nl_feature_2)
        print(type(x))
        print(x.size())
        print(x.size(0))
        output = x.view(x.size(0), -1)
        #output = self.conv_3(nl_feature_2).view(x.size(0), -1)
        print(type(output))
        print(output.size())
