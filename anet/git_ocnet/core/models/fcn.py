...
class FCN32s(nn.Module):
    ...
    def __init__(self, nclass, backbone='vgg16', aux=False, pretrained_base=True,
        ...
        if backbone == 'vgg16':
            ...
            '''
            if(pretrained_base==True):
                self.pretrained = vgg16(pretrained=pretrained_base).features
            else:
                self.pretrained = vgg16(pretrained=pretrained_base)
            '''
        else:
...
class _FCNHead(nn.Module):
    def __init__(self, in_channels, channels, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.block = nn.Sequential(
            #NONLocalBlock2D(in_channels=in_channels),#nonlocal
...
def get_fcn32s_vgg16_voc(**kwargs):
    #print('using fcn32s_vgg16_voc')
    #return get_fcn32s('pascal_voc', 'vgg16', pretrained_base=False, **kwargs)
