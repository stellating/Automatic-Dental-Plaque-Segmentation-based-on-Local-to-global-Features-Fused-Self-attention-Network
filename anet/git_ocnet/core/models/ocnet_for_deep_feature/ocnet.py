...
from .base_models.lib.non_local_embedded_gaussian import NONLocalBlock2D
#from base_models.lib.non_local_embedded_gaussian import NONLocalBlock2D

__all__ = ['OCNet', 'get_ocnet', 'get_base_ocnet_resnet101_citys',
           'get_pyramid_ocnet_resnet101_citys', 'get_asp_ocnet_resnet101_citys', 'get_base_ocnet_resnet101_teeth']
...
class OCNet(SegBaseModel):
    ...
    def forward(self, x):
        ...
        outputs = []
        #x = self.head(c4)
        x, mat_result_512 = self.head(c4)
        print('after head:')
        print('x.type:', type(x))
        print('x.size:', x.size())

        #clone x to x_for_save_3
        x_for_save_3 = x.clone()
        print('x_for_save_3.type-:', type(x_for_save_3))
        print('x_for_save_3.size-:', x_for_save_3.size())
        #trans x_for_save_3 from GPU to cpu, and change torch.tensor to numpy
        x_for_save_3 = x_for_save_3.cpu().numpy()
        #delete dim==1
        x_for_save_3 = np.squeeze(x_for_save_3)
        print('x_for_save_3.type-:', type(x_for_save_3))
        print('x_for_save_3.size-:', x_for_save_3.shape)
        #x_for_save_3 is 3*88*88
        ...
        return tuple(outputs), mat_result_512, x_for_save_3
...
class _OCHead(nn.Module):
    ...
    def forward(self, x):
        x = self.context(x)
        print('ocnet head added nonlocal!')
        print('x.type-:', type(x))
        print('x.size-:', x.size())
        #clone x to x_for_save
        x_for_save = x.clone()
        print('x_for_save.type-:', type(x_for_save))
        print('x_for_save.size-:', x_for_save.size())
        #trans x_for_save from GPU to cpu, and change torch.tensor to numpy
        x_for_save = x_for_save.cpu().data.numpy()
        #delete dim==1
        x_for_save = np.squeeze(x_for_save)
        print('x_for_save.type-:', type(x_for_save))
        print('x_for_save.size-:', x_for_save.shape)
        #x_for_save is 512*88*88
        #return self.out(x)
        return self.out(x), x_for_save
...
def get_ocnet(dataset='citys', backbone='resnet50', oc_arch='base', pretrained=False, root='~/.torch/models',
    ...
    print('datasets is: ', acronyms[dataset])
...
def get_base_ocnet_resnet101_teeth(**kwargs):
    return get_ocnet('teeth', 'resnet101', 'base', **kwargs)
